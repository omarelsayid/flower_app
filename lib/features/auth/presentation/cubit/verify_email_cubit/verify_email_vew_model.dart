...
import 'package:flower_app/features/auth/presentation/cubit/verify_email_cubit/verify_email_state.dart';
...
class VerifyEmailVewModel extends Cubit<VerifyEmailState> {
  ...
  void doIntent(VerifyEmailIntent verifyEmailIntent) {
    switch (verifyEmailIntent) {
      ...
      case ResendClickedIntent():
        _resendCode(
          SharedPreferenceServices.getData(
            AppConstants.email.toString(),
          ).toString(),
        );
  ...
  void onChanged(
    ...
    if (value.length == 1) {
      ...
      FocusScope.of(
        context,
      ).requestFocus(previousNode); // الرجوع للخلف عند المسح
    }
  }
